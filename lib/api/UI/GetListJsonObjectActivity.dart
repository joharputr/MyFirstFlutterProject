...
import 'package:flutter_app/api/model/get_model_jsonObject.dart';
import 'package:progress_dialog/progress_dialog.dart';

void main() => runApp(GetListJsonObjectActivity());

class GetListJsonObjectActivity extends StatefulWidget {
  @override
  _getDataState createState() => _getDataState();
}

class _getDataState extends State<GetListJsonObjectActivity> {
  Future<List<UserJsonObject>> getListUsers;

  @override
  void initState() {
    // here first we are checking network connection
    UserJsonObject.isConnected().then((internet) {
      if (internet) {
  ...
  Widget generateColum(UserJsonObject item) => Card(
    child: ListTile(
      leading: Image.network(item.title),
      title: Text(
        item.title,
